Figure_S1H_MYCL_Barplot <- function(Normalized_Transcript_logCPM) {
 ...
 df.melt %>% ggplot(aes(x=variable,y=value,fill=Transcript_ID)) +
            ...
            theme(axis.title.y = element_text(size=22,margin = margin(r = 70)), axis.title.x = element_text(size=22,margin = margin(t = 30)),
                    ...
                    legend.position = c(0,1), legend.justification = c(0, 1),legend.box.margin = margin(5, l = 5, unit = "mm"), plot.title = element_text(colour="black", size = 30, hjust = 0.5)) +
            guides(fill=guide_legend(nrow=3,byrow=TRUE)) -> gplot
 library(png)
 imageWidth = 3000
 imageHeight = 3000
 dpi = 300
...
print("template_function_Figure_S1H_MYCL_Barplot.R #########################################################################")
...
var_Figure_S1H_MYCL_Barplot<-Figure_S1H_MYCL_Barplot(var_Normalized_Transcript_logCPM)
invisible(graphics.off())
saveRDS(var_Figure_S1H_MYCL_Barplot, paste0(rds_output,"/var_Figure_S1H_MYCL_Barplot.rds"))

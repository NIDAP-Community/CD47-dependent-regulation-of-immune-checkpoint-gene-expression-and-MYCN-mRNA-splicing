# Batch Correction [CCBR] (4d5e38c8-27e7-4bce-a739-2120eb8c71fb): v210
Batch_Corrected_Counts_Genes_Figs_S1B_S1D <- function(Normalized_Counts_Genes, CCBR_1188_Metadata_Final) {
    ...
    #Basic Parameters:
    counts_matrix <- Normalized_Counts_Genes
    sample_metadata <- CCBR_1188_Metadata_Final
    gene_names_column <- "Gene" 
    columns_to_include <- c("Gene","gene_id","JINB8_CD3_CD28_1","JINB8_CD3_CD28_3","JINB8_CD3_CD28_6","JINB8_CD3_CD28_TSP1_1","JINB8_CD3_CD28_TSP1_3","JINB8_CD3_CD28_TSP1_6","JINB8_TSP1_1","JINB8_TSP1_3","JINB8_TSP1_6","JINB8_UT_1","Jurkat_CD3_CD28_1","Jurkat_CD3_CD28_3","Jurkat_CD3_CD28_6","Jurkat_CD3_CD28_TSP1_1","Jurkat_CD3_CD28_TSP1_3","Jurkat_CD3_CD28_TSP1_6","Jurkat_TSP1_1","Jurkat_TSP1_3","Jurkat_TSP1_6","Jurkat_UT_1")
    sample_names_column <- "Sample"
 
    groups_column <- "Cell_Line"
    covariates <- c("Cell_Line")
    batch_column <- "Batch"
...
counts_matrix[,gene_names_column]=paste0(counts_matrix[,gene_names_column],'_',1:nrow(counts_matrix))
    ...
    #Remove genes where Batch or Group variable has zero variance: 
    all_tab <- data.frame(cbind(Batch = sample_metadata[[batch_column]],as.matrix(t(df.filt))))
    all_tab %>% group_by(Batch) %>% summarise_all(var) -> all_tab_var
    var.df = data.frame(t(as.matrix(all_tab_var[,-1])))
    rownames(var.df) <- rownames(df.filt)
    colnames(var.df) <- as.character(all_tab_var$Batch)
    zerovarcols <- colnames(var.df[colSums(is.na(var.df))==nrow(var.df)])
    if(length(zerovarcols) > 0){
        var.df %>% select(-one_of(zerovarcols)) -> var.df
        sample_metadata %>% filter(.data[[batch_column]] %in% zerovarcols) %>% pull(.data[[sample_names_column]]) -> samp.removed
        sample_metadata %>% filter(!.data[[batch_column]] %in% zerovarcols) -> sample_metadata
        df.filt %>% select(-one_of(samp.removed)) -> df.filt
        cat("\nRemoved groups without batches:\n")
        print(zerovarcols)
        print(samp.removed)
        samples_to_include <- samples_to_include[!samples_to_include %in% samp.removed]
    }
    varzero = apply(var.df, 1, function(r) any(r == 0))
    cat("\nnumber of features with 0 variance per batch - removed from Batch Adjustment:\n") 
    cat(dim(df.filt[varzero == TRUE,])[1])

    if (length(covariates)==1){
        all_tab.group = data.frame(cbind(Group = sample_metadata[colnames(sample_metadata)==covariates],as.matrix(t(df.filt))))
        colnames(all_tab.group)[1] <- "Group"
        all_tab.group %>% group_by(Group) %>% summarise_all(var) -> all_tab_var.group
        group.var.df = data.frame(t(as.matrix(all_tab_var.group[,-1])))
        rownames(group.var.df) <- rownames(df.filt)
        colnames(group.var.df) <- as.character(all_tab_var.group$Group)
        
        zerogroupvarcols <- colnames(group.var.df[colSums(is.na(group.var.df))==nrow(group.var.df)])
        if(length(zerogroupvarcols) > 0){
            group.var.df %>% select(-one_of(zerogroupvarcols)) -> group.var.df
            sample_metadata %>% filter(.data[[covariates]] %in% zerogroupvarcols) %>% pull(.data[[sample_names_column]]) -> samp.group.removed
            sample_metadata %>% filter(!.data[[covariates]] %in% zerogroupvarcols) -> sample_metadata
            df.filt %>% select(-one_of(samp.group.removed)) -> df.filt
            cat("\nRemoved groups with <2 samples:\n")
            print(zerogroupvarcols)
            print(samp.group.removed)
            samples_to_include <- samples_to_include[!samples_to_include %in% samp.group.removed]
        }
        groupvarzero = apply(group.var.df, 1, function(r) any(r == 0))
        cat("\nnumber of features with 0 variance per group - also removed from Batch Adjustment:\n") 
        cat(dim(df.filt[groupvarzero == TRUE,])[1])
        cat("\n")
        df.filt = df.filt[(varzero == FALSE & groupvarzero == FALSE),]
    } else {
        df.filt = df.filt[(varzero == FALSE),]
    }
    ...
    png(
      filename="Batch_Corrected_Counts_Genes_Figs_S1B_S1D.png",
      width=imageWidth,
      height=imageHeight,
      units="px",
      pointsize=4,
...
print(colnames(combat_edata[,!colnames(combat_edata)%in%gene_names_column]))
...
print("template_function_Batch_Corrected_Counts_Genes_Figs_S1B_S1D.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_Normalized_Counts_Genes<-readRDS(paste0(rds_output,"/var_Normalized_Counts_Genes.rds"))
Input_is_Seurat_count <- 0
for(item in var_Normalized_Counts_Genes){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_Normalized_Counts_Genes<-as.data.frame(var_Normalized_Counts_Genes)}else{var_Normalized_Counts_Genes <- var_Normalized_Counts_Genes}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
var_Batch_Corrected_Counts_Genes_Figs_S1B_S1D<-Batch_Corrected_Counts_Genes_Figs_S1B_S1D(var_Normalized_Counts_Genes,var_CCBR_1188_Metadata_Final)
invisible(graphics.off())
saveRDS(var_Batch_Corrected_Counts_Genes_Figs_S1B_S1D, paste0(rds_output,"/var_Batch_Corrected_Counts_Genes_Figs_S1B_S1D.rds"))

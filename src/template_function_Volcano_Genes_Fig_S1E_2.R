# Volcano Plot - Enhanced [CCBR] (0c91aa57-0f76-4513-a063-5f9263d65727): v48
Volcano_Genes_Fig_S1E_2 <- function(DEG_Analysis_Genes) {
    # image: png
    ...
    #Basic Parameters:
    df <- DEG_Analysis_Genes
    label.col <- "Gene"
    sig.col <- "JINB8-JK_pval"
    pCutoff  = 0.001
    lfc.col <- "JINB8-JK_logFC"
    FCcutoff = 1.0
    ...
    title <- "JINB8 vs. JK"
    subtitle <- "pval <= 0.001 & |log2FC| > 1.0"
...
#CHANGE LOG (NW 2022-03-22): Export significant counts to output log, remove from plot title (~line 160)
    ...
    png(
      filename="Volcano_Genes_Fig_S1E_2.png",
      width=imageWidth,
      height=imageHeight,
      units="px",
      pointsize=4,
...
print("template_function_Volcano_Genes_Fig_S1E_2.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_DEG_Analysis_Genes<-readRDS(paste0(rds_output,"/var_DEG_Analysis_Genes.rds"))
Input_is_Seurat_count <- 0
for(item in var_DEG_Analysis_Genes){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_DEG_Analysis_Genes<-as.data.frame(var_DEG_Analysis_Genes)}else{var_DEG_Analysis_Genes <- var_DEG_Analysis_Genes}
invisible(graphics.off())
var_Volcano_Genes_Fig_S1E_2<-Volcano_Genes_Fig_S1E_2(var_DEG_Analysis_Genes)
invisible(graphics.off())
saveRDS(var_Volcano_Genes_Fig_S1E_2, paste0(rds_output,"/var_Volcano_Genes_Fig_S1E_2.rds"))
